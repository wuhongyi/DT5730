    ...
    for (b = 0; b < MAXNB; b++) {
        ...
        *      DPP parameters        *
        ...
        for(ch=0; ch<MaxNChannels; ch++) {
			...
			DPPParams[b].m[ch] = 900;      // Trapezoid Flat Top  (N*10ns) 
			DPPParams[b].M[ch] = 50000;      // Decay Time Constant (N*10ns) 
			DPPParams[b].ftd[ch] = 500;    // Flat top delay (peaking time) (N*10ns) 
			...
			DPPParams[b].twwdt[ch] = 100;  //Rise Time Validation Window (N*10 ns)
        }
    }
