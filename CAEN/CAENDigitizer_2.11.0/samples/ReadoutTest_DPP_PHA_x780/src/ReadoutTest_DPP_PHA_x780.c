    ...
    for (b = 0; b < MAXNB; b++) {
        ...
        *      DPP parameters        *
        ...
        for(ch=0; ch<MaxNChannels; ch++) {
			...
			DPPParams[b].k[ch] = 130;//2000;     // Trapezoid Rise Time (N*10ns)
			DPPParams[b].m[ch] = 600;//1000;      // Trapezoid Flat Top  (N*10ns) 
			DPPParams[b].M[ch] = 270;//50000;      // Decay Time Constant (N*10ns) 
			DPPParams[b].ftd[ch] = 600;//800;    // Flat top delay (peaking time) (N*10ns) 
            DPPParams[b].a[ch] = 4;       // Trigger Filter smoothing factor (number of samples to average for RC-CR2 filter) Options: 1; 2; 4; 8; 16; 32
			DPPParams[b].b[ch] = 120;//200;     // Input Signal Rise time (N*10ns)
			...
			DPPParams[b].pkho[ch] = 2000;  //peak holdoff (N * 1 ns)
			DPPParams[b].blho[ch] = 500;   //Baseline holdoff (N*10 ns)
			...
			DPPParams[b].twwdt[ch] = 0;  //Rise Time Validation Window (N*10 ns)
        }
    }
